...
run_event <- function(data, treat_var, base_q, outcome_var, label) {
    # FE de HH absorve heterogeneidade entre domicilios e o main effect
    # de treat_var (que eh constante dentro de cada HH com classif. estavel).
    # Identifica efeito apenas da variacao intra-HH ao longo do tempo.
    f <- as.formula(sprintf(
        "%s ~ i(yr_q, %s, ref = '%s') | hh_id + yr_q",
        outcome_var, treat_var, base_q
    ))
